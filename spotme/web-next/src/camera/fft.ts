/**
 * Radix-2 FFT — TypeScript port of the CAM-1 `fft.js` (#56, read-only
 * source), written in-repo because phase correlation needs one and the
 * engine ships zero dependencies. Iterative in-place Cooley–Tukey with
 * bit-reversal; 2D as rows then columns. Sizes must be powers of two —
 * asserted, never quietly padded. Inverse carries the 1/N scaling so
 * fft→ifft round-trips exactly (to float precision), which the tests pin.
 */
...
/** In-place 1D FFT over separate re/im Float32Arrays. */
...
export function fft2d(re: Float32Array, im: Float32Array, width: number, height: number, inverse = false): void {
  if (!isPow2(width) || !isPow2(height)) {
    throw new Error(`fft2d needs power-of-two dimensions, got ${width}x${height}`);
  }
