...
export default [
  {
    // Build output, not source. `eslint .` walks these when a local Android or
    // Vite build has run, and reports rules the config never declares -- red
    // locally, green in CI's fresh clone, which is the worst of both.
    ignores: ['android/**', 'dist/**', 'vendor/**', 'node_modules/**'],
  },
