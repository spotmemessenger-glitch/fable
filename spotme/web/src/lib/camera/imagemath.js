/**
 * Spot Me camera — plane and pixel math shared by HDR, night stack,
 * alignment and stabilization.
 *
 * Two shapes flow through this file:
 ...
 * which is what lets the golden-vector tests pin exact numbers in Node.
 */
...
export function downscalePlane (plane, targetW, targetH) {
 ...
 *  right/down. */
export function shiftImage (image, dx, dy) {
  const { data, width, height } = image
...
export function shiftPlane (plane, dx, dy) {
  const { data, width, height } = plane
...
export function boxBlurPlane (plane, radius) {
...
/** RGBA box blur (radius r), edge-clamped, alpha preserved — the portrait
 *  background blur primitive. `passes` ≥ 3 approximates a Gaussian. */
export function boxBlurImage (image, radius, passes = 1) {
  let current = image
  for (let p = 0; p < passes; p++) current = boxBlurOnce (current, radius)
  return current
}

function boxBlurOnce (image, radius) {
...
export function planeVariance (plane) {
