 ...
 *   POST /api/username {username,id,name} -> 200 {ok:true} | 409 {error:'taken'}
 */
...
async function handleClaim (req, res) {
  ...
  const putRes = await fetch(`${BLOB_API}/${pathnameFor(username)}`, {
    ...
    body: JSON.stringify({ username, id, name: displayName, ts: Date.now() })
  })
...
export default async function handler (req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') { await handleClaim(req, res); return }
