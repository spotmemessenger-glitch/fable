...
| Camera suite CAM-1…4 | Implemented (Draft PR **#56/#58/#59/#55**, `feat/camera-engine` + stacked) | **freeze LIFTED for web integration only (ADR-029)** — branches/PRs are read-only SOURCE MATERIAL for the Stage 1 migration; PRs left open for the owner |
| **Camera Stage 1 C1** — camera contracts v1 (privacy-by-absence capture shapes — no location/exif field compiles; closed refusal vocabulary refusing fake HDR/slow-mo; edit-op graph with undo-as-slicing; vision scan + honest OCR states; `advanced` beauty structurally requiring the approved MediaPipe landmarker literal; single-request-only `CloudConsentContext`; `ModelAssetRecord` integrity shape) + ADR-029 (freeze lift + engine bundle) + DECISIONS record + threat model T-CA-1..8 + programme with the Phase 0 audit table | **Implemented (Draft PR — DARK)** | `packages/contracts/src/camera.ts`, `packages/contracts/test/camera-{negative,usage}.test.ts`, `docs/adr/029-camera-engine-choices-freeze-lift.md`, `docs/architecture/camera-platform/01-CONTRACTS-THREAT-MODEL.md`, `docs/handbook/PLATFORM-CAMERA-STAGE-1-PROGRAMME.md` |
