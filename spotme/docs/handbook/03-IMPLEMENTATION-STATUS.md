...
| **Platform Phase 5B** — dark media pipeline: M1 media ports on the existing `IStorageAdapter`, EXIF/GPS strip proven before persistence, M8 reserved queues (`{moment-media}`/`{story-expiry}`/`{feed-refresh}`/`{moderation}`) as contracts + fixture workers (inert without env), sha256 dedup + refCount cascade | Implemented (Draft PR — DARK) | branch `feat/platform-phase-5b-media-pipeline`; `backend/src/moment-media/`, migration `20260804200000_moment_media` — `MediaModule` NOT imported |
| **Platform Phase 5C** — dark MomentsModule: M5 four-tier visibility (private never in any feed SQL), three feeds chronological-first + M2 frozen registry (forbidden throws by name), M3 six-state stories + `{story-expiry}`, M4 flat comments + closed reactions (DB CHECK), two-way block in SQL, moderation machine + sanitized audit + child-safety priority lane, Disabled realtime | Implemented (Draft PR — DARK) | branch `feat/platform-phase-5c-moments-backend`; `backend/src/moments/`, migration `20260804210000_moments_backend` — `MomentsModule` NOT imported |
| **Platform Phase 5D** — inert web-next Moments surface: two-step M5 location attach (plain-language explanation before any attach), untrusted URLs (no unfurl), closed reaction bar, flat-data comments, report/block/hide, no engagement counters, virtualized 3-mode feed, CameraPort interface-only | Implemented (Draft PR — DARK) | branch `feat/platform-phase-5d-moments-webnext`; `web-next/src/moments/` — App unchanged |
| **Platform Phase 5E** — Moments dark fences + full M9 battery (import-graph/dependency/ranking-invariant/storage-boundary/artifact scans, EXIF re-proof, private-never-in-projection), closed metrics, benchmark (100k achieved, honest linear-growth flag), runbooks (child-safety mandatory path), activation checklist (moderation staffing + legal review HARD) | Implemented (Draft PR — DARK) | branch `feat/platform-phase-5e-moments-fences-perf-ops-docs`; `backend/test/moments-dark-fences.spec.ts`, `docs/architecture/moments-platform/` |
