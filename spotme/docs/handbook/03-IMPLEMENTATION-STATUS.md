...
| **Map tiles — MapLibre GL + self-hosted PMTiles on R2** (ADR-030; `TILES_URL`; no third-party tile host, fence-tested; owner runbook for the India extract) | Implemented (Draft PR **#117**, `feat/map-tiles-selfhosted`) | `web-next/src/map/`, fixture `web-next/test/fixtures/sample.pmtiles`, artifact fence `web-next/scripts/check-map-artifact.mjs` — dark: nothing mounts it; bundle byte-identical while dark |
| **Product analytics — AnalyticsPort + closed vocabulary + PostHog adapter** (ADR-031; flag+key gated; privacy laws compile-time + runtime + artifact fenced) | Implemented (Draft PR **#118**, `feat/analytics-posthog`) | `packages/contracts/src/analytics.ts`, `web-next/src/analytics/`, `web-next/scripts/check-analytics-artifact.mjs` — dark: NOOP sink, flag false, no key |
| **Bot protection — Turnstile on signup/guest/OTP** (ADR-032; structural bypass without `TURNSTILE_SECRET_KEY`, timeout-safe fail-open on outage, fail-closed verdicts; invisible widget + a11y fallback) | Implemented (Draft PR **#119**, `feat/auth-turnstile`) | `backend/src/middleware/turnstile.ts` + `auth.module.ts` wiring, `web/src/lib/turnstile.js`; units + real-HTTP e2e + web gate fence — dark: keys absent everywhere |
