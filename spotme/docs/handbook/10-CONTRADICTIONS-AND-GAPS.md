...
| C2 | Discovery "location honesty" was described as fully approximate while the v1 lobby still broadcast **precise GPS** on `master`; the approximate-only model lived only in draft PR #60. | `web/src/lib/discovery.js:49` on master; guard `web/test/discovery-coarse-broadcast.test.js` | **RESOLVED 2026-08-03:** the precise-GPS broadcast was fixed on `master` by the **coarse-location hotfix (PR #66, merge `069905e`, [ADR-024](../adr/024-discovery-coarse-broadcast-hotfix.md))** — public presence now emits only `coarse()` output, guarded by a regression test. The broader approximate-only model (#60) remains draft and rebases onto ADR-024. C2 is closed. |
...
| C4 | **ADR-021 records a four-step** Discovery sequence; **Product Roadmap v2.0 records five** (SpotMe Exchange inserted at step 2, owner decision 2026-08-03). | `../adr/021-*.md` vs `product/SPOT-ME-PRODUCT-ROADMAP-V2.md §12` | **RESOLVED 2026-08-03:** ADR-022 (five-step sequence) and ADR-023 (Intent Graph) were ratified by the owner and are Accepted. ADR-021 remains immutable and otherwise in force; its sequence portion is superseded by ADR-022. C4 is closed. |
...
| A5 | The **approved SpotMe Exchange specification** was **not present** in any available source. A **dedicated Exchange PRD** has now been authored (`product/exchange/`) as a **ratifiable reconstruction** from the owner's named components + established principles — this replaces "no spec" with "a draft spec to ratify". **A5 stays open until the owner ratifies** (checklist: `exchange/13-ACCEPTANCE-AND-OPEN-QUESTIONS.md §13.1`). | `product/exchange/`; roadmap v2.0 §14 | Owner: ratify or correct the PRD (esp. `[PROPOSED]` values); provide the verbatim approved spec if one exists; then mark A5 closed. Do not begin Exchange implementation until ratified. |
