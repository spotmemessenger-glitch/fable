...
| `POST /v2/auth/signing-key/supersede` | Body: new key + `supersessionSigB64` — the OLD key's signature over the length-prefixed transcript `('spotme-signing-supersede-v1', userId, oldPublicKeyB64, newPublicKeyB64)`, built with the exact `transcript()` framing from `signing-identity.js` (uint32be field count + per-field length prefixes; JSON and delimiter-joins are forbidden for signed structures, per the #29 review). The server verifies against the STORED old public key before touching the chain; a bad signature is a 400 and the chain is untouched. Old row → `superseded` + pointer + sig; new row → `active`. |
