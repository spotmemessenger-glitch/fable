...
**Companion:** `004a-e2e-v3-envelope-schema.md` specifies the wire format, the
version negotiation and downgrade rules, the legacy-compatibility contract and
the test plan — everything this ADR defers. Its test vectors are regenerated by
`004a-e2e-v3-vectors.mjs`. Neither is a decision to build: this ADR's two
